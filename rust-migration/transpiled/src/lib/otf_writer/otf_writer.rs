...
unsafe extern "C" fn serializeToOTF(
        ...
        == FONTTYPE_TTF as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        ...
        otfcc_SFNTBuilder_pushTable(builder, 1819239265i32 as uint32_t, pair.loca);
    } else {
        ...
        == FONTTYPE_TTF as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        ...
        otfcc_SFNTBuilder_pushTable(
            builder,
            1886545264i32 as uint32_t,
            otfcc_buildFpgmPrep((*font).prep, options),
        );
        otfcc_SFNTBuilder_pushTable(
            builder,
            1668707360i32 as uint32_t,
            otfcc_buildCvt((*font).cvt_, options),
        );
        otfcc_SFNTBuilder_pushTable(
            builder,
            1280594760i32 as uint32_t,
            otfcc_buildLTSH((*font).LTSH, options),
        );
        otfcc_SFNTBuilder_pushTable(
            builder,
            1447316824i32 as uint32_t,
            otfcc_buildVDMX((*font).VDMX, options),
        );
    }
    if !(*font).hhea.is_null() && !(*font).maxp.is_null() && !(*font).hmtx.is_null() {
        let mut hmtx_counta: uint16_t = (*(*font).hhea).numberOfMetrics;
        let mut hmtx_countk: uint16_t = ((*(*font).maxp).numGlyphs as ::core::ffi::c_int
            - (*(*font).hhea).numberOfMetrics as ::core::ffi::c_int)
            as uint16_t;
        otfcc_SFNTBuilder_pushTable(
            builder,
            1752003704i32 as uint32_t,
            otfcc_buildHmtx(
                (*font).hmtx,
    ...
    otfcc_SFNTBuilder_pushTable(
        builder,
        1986553185i32 as uint32_t,
        otfcc_buildVhea((*font).vhea, options),
    );
    if !(*font).vhea.is_null() && !(*font).maxp.is_null() && !(*font).vmtx.is_null() {
        let mut vmtx_counta: uint16_t = (*(*font).vhea).numOfLongVerMetrics;
        let mut vmtx_countk: uint16_t = ((*(*font).maxp).numGlyphs as ::core::ffi::c_int
            - (*(*font).vhea).numOfLongVerMetrics as ::core::ffi::c_int)
            as uint16_t;
        otfcc_SFNTBuilder_pushTable(
            builder,
            1986884728i32 as uint32_t,
            otfcc_buildVmtx(
                (*font).vmtx,
    ...
    otfcc_SFNTBuilder_pushTable(
        builder,
        1195656518i32 as uint32_t,
        otfcc_buildGDEF((*font).GDEF, options),
    );
    otfcc_SFNTBuilder_pushTable(
        builder,
        1111577413i32 as uint32_t,
        otfcc_buildBASE((*font).BASE, options),
    );
    otfcc_SFNTBuilder_pushTable(
        builder,
        1129333068i32 as uint32_t,
        otfcc_buildCPAL((*font).CPAL, options),
    );
    otfcc_SFNTBuilder_pushTable(
        builder,
        1129270354i32 as uint32_t,
        otfcc_buildCOLR((*font).COLR, options),
    );
    otfcc_SFNTBuilder_pushTable(
        builder,
        1398163232i32 as uint32_t,
        otfcc_buildSVG((*font).SVG_, options),
    );
    let mut target: tsi_BuildTarget = otfcc_buildTSI((*font).TSI_01, options);
    otfcc_SFNTBuilder_pushTable(builder, 1414744368i32 as uint32_t, target.indexPart);
    otfcc_SFNTBuilder_pushTable(builder, 1414744369i32 as uint32_t, target.textPart);
    let mut target_0: tsi_BuildTarget = otfcc_buildTSI((*font).TSI_23, options);
    otfcc_SFNTBuilder_pushTable(builder, 1414744370i32 as uint32_t, target_0.indexPart);
    otfcc_SFNTBuilder_pushTable(builder, 1414744371i32 as uint32_t, target_0.textPart);
    if !(*font).glyf.is_null() {
        otfcc_SFNTBuilder_pushTable(
            builder,
            1414744373i32 as uint32_t,
    ...
    if (*options).dummy_DSIG {
        ...
        bufwrite32b(dsig, 0x1 as uint32_t);
        bufwrite16b(dsig, 0 as uint16_t);
        bufwrite16b(dsig, 0 as uint16_t);
        otfcc_SFNTBuilder_pushTable(builder, 1146308935i32 as uint32_t, dsig);
    }
